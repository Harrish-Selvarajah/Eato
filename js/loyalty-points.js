...
$(document).ready(function () {
    ...
    renderActivity();
    $('#loyaltyPoints').text(Math.round(loyaltyPoints) + ' Points');
});
...
function renderActivity() {
    ...
    loyaltyArray.forEach(function (item) {
        ...
        <div style="text-align: center;"><span class="subtitle">You earned ${Math.round(item.points)} points</span> <br><span style="color: #5C5C5C;" class="primaryText"> Rs ${item.price} at ${item.via}</span></div>
        <div class="acitivity_circle" style="border: 1px solid  black; "><span class="subtitle">+${Math.round(item.points)}</span></div>
    </div> `
        } else {
            if (item.method === 'QR') {
                renderHtml += `<div class="acitivity card">
